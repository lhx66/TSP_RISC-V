...
core_list_init(ee_u32 blksize, list_head *memblock, ee_s16 seed)
{
    ...
    ee_u32 per_item = sizeof(list_head) + sizeof(list_data);  /* 【修复】：正确的计算方法 */
    ee_u32 size     = (blksize / per_item) - 2;

    /* 添加关键调试信息 */
    ee_printf("[DEBUG] Init: size=%u\r\n", size);
    ...
    ee_printf("[DEBUG] Starting list insertion: size=%d\r\n", size);
    ...
    ee_printf("[DEBUG] Initial tail found: %p\r\n", tail);
    ...
    for (i = 0; i < size; i++)
    {
        ...
        // 在尾部插入新节点，并更新tail指针
        list_head *new_node = core_list_insert_new(
            tail, &info, &memblock, &datablock, memblock_end, datablock_end);
        if (new_node != NULL) {
            tail = new_node;  // 更新tail到新插入的节点
        }

        // 每10个节点验证一次链表
        if ((i % 10) == 0) {
            int count = 0;
            list_head *check = list;
            while (check != NULL && count < 20) {
                count++;
                check = check->next;
            }
            ee_printf("[DEBUG] Inserted %d nodes, current depth: %d, tail: %p\r\n", i+1, count, tail);
        }
    }

    // 验证最终结果
    ee_printf("[DEBUG] After insertion: list=%p, list->next=%p\r\n", list, list->next);
    ...
    {
        if (loop_count++ > 10000) {  // 防止死循环
            ee_printf("[ERROR] Indexing safety limit exceeded!\r\n");
            while(1);
        }
    ...
    ee_printf("[DEBUG] Indexed %d nodes\r\n", loop_count);

    // 【关键调试】：验证链表结构
    ee_printf("[DEBUG] Verifying list structure...\r\n");
    finder = list;
    int verify_count = 0;
    while (finder != NULL && verify_count < 100) {
        verify_count++;
        finder = finder->next;
    }
    ee_printf("[DEBUG] Verify: %d nodes\r\n", verify_count);
                     ...
                     list_data * datablock_end)
{
    ...
    if (*memblock >= memblock_end) {
        return NULL;
    }
    if (*datablock >= datablock_end) {
        return NULL;
    }
...
core_list_mergesort(list_head *list, list_cmp cmp, core_results *res)
{
    ...
    ee_printf("[DEBUG] core_list_mergesort: Starting sort...\r\n");
    insize = 1;
    int pass_count = 0;

    while (1)
    {
        if (pass_count++ > 20) {  // 防止死循环
            ee_printf("[ERROR] mergesort: Too many passes (>20), possible infinite loop!\r\n");
            return list;
        }
        ...
        while (p)
        {
            ...
            while (psize > 0 || (qsize > 0 && q))
            {
                ...
                if (psize == 0)
                {
                    ...
                    e = q;
                    if (!e) {
                        ee_printf("[ERROR] mergesort: e is NULL! qsize=%d\r\n", qsize);
                        return list;
                    }
                    q = q->next;
                    qsize--;
                ...
                else if (qsize == 0 || !q)
                {
                    ...
                    e = p;
                    if (!e) {
                        ee_printf("[ERROR] mergesort: e is NULL! psize=%d\r\n", psize);
                        return list;
                    }
                    p = p->next;
                    psize--;
                ...
                else if (cmp(p->info, q->info, res) <= 0)
                {
                    ...
                    e = p;
                    if (!e) {
                        ee_printf("[ERROR] mergesort: e is NULL! p=%p\r\n", p);
                        return list;
                    }
                    p = p->next;
                    psize--;
                ...
                else
                {
                    ...
                    e = q;
                    if (!e) {
                        ee_printf("[ERROR] mergesort: e is NULL! q=%p\r\n", q);
                        return list;
                    }
                    q = q->next;
                    qsize--;
