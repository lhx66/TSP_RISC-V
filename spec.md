...
- `template/tools/bin2txt.py` supports `--elf` and extracts `.text` plus `.data` with `riscv-none-elf-objcopy`. It emits Pango-compatible word-per-line initialization images. The SRAM image is zero-padded to the complete 32 KiB DTCM, so it initializes `.data/.rodata` and leaves the remaining SRAM deterministic before startup clears `.bss`.
- `build_and_convert.bat coremark` now emits `FPGA/pango_cpu/source/coremark_iram.dat` (4,789 words, 19,156 bytes) and `coremark_sram.dat` (8,192 words, 32 KiB). The previous single `RTL/sim/programs/coremark.hex/.txt` files were removed because they incorrectly mixed the IRAM and SRAM images.
